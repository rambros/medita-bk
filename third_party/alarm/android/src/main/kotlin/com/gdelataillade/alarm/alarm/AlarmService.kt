...
class AlarmService : Service() {
    companion object {
        ...
        // Channel ID shared with NotificationHandler to avoid creating a duplicate channel.
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        // CRITICAL FIX — Android 14+ (API 34+) throws ForegroundServiceDidNotStartInTimeException
        // if startForeground() is not called within ~5 seconds of startForegroundService().
        // We must call startForeground() here, before any error checks or heavy work,
        // so the system never triggers that timeout regardless of what happens next.
        val earlyId = intent?.getIntExtra("id", 0)?.takeIf { it != 0 } ?: startId
        startEarlyForeground(earlyId)
        ...
        // Update the foreground notification with the real alarm notification.
        // (startForeground was already called above with a minimal placeholder.)
        try {
    ...
    /**
     * Starts the service as foreground immediately with a minimal placeholder notification.
     *
     * This MUST be called at the very start of onStartCommand() to satisfy Android 14+
     * (API 34+) requirement that startForeground() is called within ~5 seconds of
     * startForegroundService(). Failure to do so results in
     * ForegroundServiceDidNotStartInTimeException, which crashes the app.
     *
     * The notification is replaced moments later with the real alarm notification once
     * the alarm settings are parsed and buildNotification() completes.
     */
    private fun startEarlyForeground(id: Int) {
        try {
            // Ensure the notification channel exists. NotificationHandler creates the same
            // channel on its own init, but it may not have run yet at this point.
            ...
            val notification = NotificationCompat.Builder(this, CHANNEL_ID)
                ...
                .setContentText("Preparando alarme...")
            ...
            Log.e(TAG, "startEarlyForeground failed: ${e.message}", e)
        }
    }
