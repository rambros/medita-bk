...
Future initFirebase() async {
  if (kIsWeb) {
    ...
    if (firebaseApiKey.isEmpty) {
      throw StateError('FIREBASE_WEB_API_KEY is not set. Provide it via --dart-define for web builds.');
    }

    const authDomain = String.fromEnvironment('FIREBASE_AUTH_DOMAIN', defaultValue: _defaultAuthDomain);
    const projectId = String.fromEnvironment('FIREBASE_PROJECT_ID', defaultValue: _defaultProjectId);
    const storageBucket = String.fromEnvironment('FIREBASE_STORAGE_BUCKET', defaultValue: _defaultStorageBucket);
    const messagingSenderId =
        String.fromEnvironment('FIREBASE_MESSAGING_SENDER_ID', defaultValue: _defaultMessagingSenderId);
    const appId = String.fromEnvironment('FIREBASE_APP_ID', defaultValue: _defaultAppId);
    const measurementId = String.fromEnvironment('FIREBASE_MEASUREMENT_ID', defaultValue: _defaultMeasurementId);
